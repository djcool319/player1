 ...
 async execute(client, interaction) {
  try {
    ...
    if (!interaction.channel.name.includes('タイムライン')) {
      await interaction.reply({ content: 'このコマンドは「タイムライン」チャンネルでのみ使用できます。', ephemeral: true });
      return;
    }
    ...
    const userName = isAnonymous ? '匿名' : (await interaction.guild.members.fetch(interaction.user.id)).displayName ?? interaction.user.username;
